...
public:
	...
	API_HANDLER_ENDPOINT_QUERY_AUTH(API_M_GET, "/sales-contract/get-sales-contract-list/{query}", querySalesContractList, SalesContractPageQuery::Wrapper, execQuerySalConListGT(query));	//定义获取获取销售合同列表接口描述
	...
	//生成出库单 gugu
	API_HANDLER_ENDPOINT_AUTH(API_M_POST, "/sales-contract/Generate-Outbound-Order", GenerateOutboundOrder, BODY_DTO(GenerateOutboundOrderDTO::Wrapper, dto), execGenerateOutboundOrder(dto, authObject->getPayload()));
	ENDPOINT_INFO(GenerateOutboundOrder) {
		// 定义接口标题
		API_DEF_ADD_TITLE(ZH_WORDS_GETTER("contract.controller.GenerateOutboundOrder"));	//目录名.controller.方法名.summary
		// 定义默认授权参数（可选定义，如果定义了，下面ENDPOINT里面需要加入API_HANDLER_AUTH_PARAME）
		API_DEF_ADD_AUTH();
		// 定义响应参数格式	
		API_DEF_ADD_RSP_JSON_WRAPPER(BooleanJsonVO);
		// 定义其他查询参数描述
		API_DEF_ADD_QUERY_PARAMS(String, "name", ZH_WORDS_GETTER("customer.field.name"), "li ming", false);
		API_DEF_ADD_QUERY_PARAMS(String, "source", ZH_WORDS_GETTER("customer.field.source"), u8"网络", false);
...
private:
	...
	//生成出库单 gugu
	BooleanJsonVO::Wrapper execGenerateOutboundOrder(const GenerateOutboundOrderDTO::Wrapper& dto, const PayloadDTO& payload);
	//导出销售合同 科文
